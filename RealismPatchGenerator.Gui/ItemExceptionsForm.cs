...
internal sealed class ItemExceptionsForm : Form
{
    ...
    {
        ["Title"] = new("例外物品管理", "Exception Item Manager"),
        ["Intro"] = new("流程调整为：第一步只按 Name 搜索 output 里的现有输出物品；载入后，右侧会显示当前属性，你可以在同一区域新增、修改、删除字段，最后单独保存这一件例外物品。", "Workflow: step 1 searches only existing output items by Name. After loading, the right side shows the current properties so you can add, edit, and remove fields before saving that exception item."),
        ["SearchGroup"] = new("第一步：按名称搜索物品", "Step 1: Search Exception Item By Name"),
        ["SavedGroup"] = new("已保存的例外物品", "Saved Exception Items"),
        ["EditorGroup"] = new("第二步到第四步：加载、编辑并保存物品", "Steps 2-4: Load, Edit, and Save Exception Item"),
        ...
        ["LoadSelectedSaved"] = new("载入已保存项", "Load Saved Exception Item"),
        ...
        ["SaveItem"] = new("保存物品", "Save Exception Item"),
        ["DeleteItem"] = new("删除物品", "Delete Exception Item"),
        ["Close"] = new("关闭", "Close"),
        ...
        ["Saved"] = new("当前物品的例外配置已保存。", "The current exception item has been saved."),
