...
SPT Realism Value Range Generator v1.30.0 is a toolset designed specifically for SPT Realism Mod 1.6.4. It is used to edit realism value rules, generate realism patches, check generated results, and apply field-level exceptions to specific items.

SPT 现实主义数值范围编辑生成器 v1.30.0 是专门为SPT Realism Mod 1.6.4设计的工具集，用于编辑现实主义数值规则、生成现实主义补丁、检查生成结果，并能对特定物品应用字段级例外覆盖。

This tool brings the GUI editor, rule files, item exception management, and output checking together into one program. It is intended for users who want to maintain realism value rules in a more structured way, generate stable patch results, and continue verifying whether the generated output still follows the expected ranges.

本工具将 GUI 编辑器、规则文件、例外物品管理和输出检查统一收拢到同一个程序中。它适合希望以更结构化方式维护现实主义数值规则、生成稳定补丁结果，并在生成后继续验证输出是否遵循预期范围的用户。
...
- Generate patch output from the GUI.
- 通过 GUI 生成补丁输出。
...
- Fixed-random-seed generation in the GUI for reproducible runs.
- Fixed-random-seed generation in the GUI for reproducible runs.
- GUI 支持固定随机种子生成，可用于复现同一轮结果。
...
- 支持通过 RealismItemRules/item_exceptions.json 对个别物品做字段级例外覆盖。

## Release Packages / 发布包选择

- Full package: includes runtime dependencies, larger size, recommended for most users.
- 完整包：包含运行时依赖，体积更大，适合绝大多数用户直接使用。
- Lightweight package: smaller size, requires matching .NET Desktop Runtime already installed on the target machine.
- 轻量包：体积更小，但要求目标机器已经安装匹配的 .NET Desktop Runtime。
- Current release output includes both package types side by side so users can choose based on their environment.
- 当前发布会同时提供完整包和轻量包，方便用户根据自己的运行环境直接选择。

- Audit now validates both numeric ranges and template-driven output structure across weapons, attachments, ammo, and gear.
- 审计现在会同时验证数值范围与模板驱动的输出结构，覆盖武器、附件、弹药和装备四大类。
