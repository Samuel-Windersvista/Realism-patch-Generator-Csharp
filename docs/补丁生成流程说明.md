...
2. CLI 程序
   - 文件：RealismPatchGenerator.Cli/Program.cs
   - 支持 basePath、outputPath、seed 与 include 过滤参数
   - 本质上也是调用同一个 Core 生成器

也就是说，无论你是从 GUI 运行，还是从 CLI 运行，真正负责生成补丁的都是：
...
其中 consumables 模板也会随模板库一起预加载，用于结构兜底与字段边界参考；但当前主规则修正重点仍然是武器、附件、弹药、装备四大类。
...
从“命名格式”看，当前正式支持 12 种输入格式；从“构建路由家族”看，这 12 种格式会收敛到 4 条主路径：

- RealismStandardTemplate：条目包含 $type 和 ItemID，且不带旧格式标记字段；条目可以额外带 TemplateID，表示这是标准模板内部的克隆引用，而不是 legacy 输入

- itemTplToClone family：包括 WttArmory_templates、Epic_templates、ConsortiumOfThings_templates、Requisitions_templates、EcoAttachment_templates、Artem_templates、WttStandalone_templates、SptBattlepass_templates；它们共享 itemTplToClone / 模板提示驱动的构建路径，但会按来源子类补充各自的 parent/template 提示

- Moxo / Mixed family：Moxo_Template 条目包含 clone，且同时包含 item 或 items；Mixed_templates 整个文件属于 legacy item/items 体系，其中既允许 clone + item/items，也允许 direct item/items
...
### 21.1 当前正式支持 12 种输入格式，但会收敛到 4 条主构建路径

也就是文档开头列出的十二种命名格式；其中 WTT 八个子类共享 itemTplToClone 家族路由，Moxo 与 Mixed 共享 legacy item/items 家族处理，RaidOverhaul 走 ItemToClone 家族，标准模板走 RealismStandardTemplate 家族。
